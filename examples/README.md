...
NVADER can export full ReAct execution traces for debugging, evaluation, and auditability:

```bash
nvader agent "Summarize the certification coverage" \
  --knowledge-path docs \
  --max-steps 5 \
  --trace-output data/traces/sample-agent-run.json
```

The trace includes steps, tool calls, observations, errors, retry counts, and final answer metadata.
